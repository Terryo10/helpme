jQuery(document).ready(function ($) {
    ...
    function bindEvents() {
        ...
        paymentMethodRadios.on('change', function () {
            ...
            $(this).closest('.payment-method-option').find('.payment-method-card').addClass('selected');
            validateCurrentStep();
        });
        ...
        if (currentStep === totalSteps) {
            ...
            switch (currentStep) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    nextButton.html('<span>' + helpmeDonations.i18n.enter_details + '</span>');
                    break;
                case 4:
    ...
    function loadGatewayPaymentForm() {
        const selectedGateway = form.find('input[name="payment_gateway"]:checked').val();
                ...
                if (response.success) {
                    ...
                    container.html(`
                        <div class="error-message">
                            <p>${response.data.message || helpmeDonations.i18n.payment_form_error}</p>
                        </div>
                    `);
                }
            },
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success && response.data.client_secret) {
                    ...
                    showMessage(response.data.message || 'Payment processing failed', 'error');
                }
            },
            error: function () {
    ...
    function processPayPalPayment() {
        ...
        showMessage('Please use the PayPal button above to complete your payment', 'info');
    }
    ...
    function paymentCompleted(paymentData) {
        ...
        form.find('.final-amount').text(formatCurrency(selectedAmountInput.val(), form.find('input[name="currency"]').val()));
    ...
    function resetForm() {
        ...
        donorDetails.css('opacity', '1');
        form.find('#donor-name, #donor-email').prop('required', true);
        $('#gateway-payment-container').html('<div class="payment-form-placeholder"><p>Please select a payment method to continue.</p></div>');
        updateStepDisplay();
        updateNavigationButtons();
        messagesContainer.empty();
    }
